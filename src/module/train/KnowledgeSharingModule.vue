        ...
        <a-row style="margin-bottom: 10px;margin-top:10px;">
          <a-col>
            ...
            <a-input-search
              ...
              @search="doQueryKnowledgeShare"
              @pressEnter="doQueryKnowledgeShare"/>
            <a-button type="primary" icon="search" @click="doQueryKnowledgeShare" style="margin-left: 30px">查询</a-button>
...
import { doLoadKnowledgeShareList, exportKnowledgeShareTemplate, importKnowledgeShareData, exportShareErrorRecords, doDeleteShareData } from '@/api/learnshare'
import moment from 'moment/moment'
import { downFile } from '@/utils/util'
import store from '@/store'
export default {
  name: 'KnowledgeSharingModule',
  components: {
    STable,
    MemorySelect
  },
  data () {
    return {
      ...
      loadTableList: parameter => {
        return doLoadKnowledgeShareList(Object.assign(parameter, this.queryParam)).then(res => {
          if (res.code === 200) {
  ...
  methods: {
    ...
    handleDeptChange (val) {
      ...
      this.doQueryKnowledgeShare()
    },
    doQueryKnowledgeShare () {
      this.rowSelected = ''
      setTimeout(() => {
    ...
    doUploadData () {
      ...
      const me = this
      importKnowledgeShareData(formData).then(res => {
        if (res.code === 200) {
    ...
    doDownloadTemplate () {
      ...
      exportKnowledgeShareTemplate(params).then(res => {
    ...
    doDownloadErrorRecords () {
      ...
      exportShareErrorRecords().then(res => {
        const headers = res.headers
        const contentType = headers['content-type']
        if (res.data) {
          const blob = new Blob([res.data], { type: contentType })
    ...
    doBatchRemove () {
      ...
      this.$confirm({
        ...
        onCancel () {
          doDeleteShareData({ deletes: me.selectedRowKeys }).then(res => {
            if (res.code === 200) {
